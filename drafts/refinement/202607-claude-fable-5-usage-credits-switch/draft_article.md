...
## 先に結論：7月7日まではサブスク内、7月8日以降はusage creditsへ
## そもそも何が変わるのか
...
**Claude Fable 5は7月7日までサブスク内で使えますが、週次上限の最大50%までで、7月8日以降はusage credits経由になる予定です。**

- 7月7日までは、Pro / Max / Team / 一部EnterpriseでFable 5をサブスク枠内でも利用可能
- ただし、使えるのは週次上限の最大50%までで、無制限ではない
- 7月8日以降は、公式説明ではusage creditsを使う形に切り替わる
...
Fable 5は復活しましたが、サブスク内でそのまま長く使える形ではありません。公式説明では、7月7日までは一部の有料プランでサブスク枠内に含まれ、7月8日以降はusage credits経由になるとされています。
...
スマホ料金でたとえるなら、基本プランに含まれる通信量を使い切ったあと、追加データを購入する感覚に近いです。ただし、Claudeの場合は通信量ではなく、モデルの処理量に応じて消費されます。
...
usage creditsを有効にすると、上限後も追加で使えるようになる一方、課金管理が必要になります。特に、Fable 5のような高性能モデルを使う場合は、思ったよりcreditsを消費する可能性があります。
...
| 復帰時期 | 未発表 |
| 7月8日以降の扱い | usage credits経由になる予定 |
| 実画面の挙動 | 切り替え後に確認が必要 |
...
ただし、公式本文だけで日本時間の厳密な切り替え時刻まで確認できるわけではありません。そのため、記事では「公式では7月7日まで」「報道では米国太平洋時間ベース」と分けて書くのが安全です。

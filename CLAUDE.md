...
### 記事冒頭は軽く（説明ブロックを重ねない・2026-05-31）

冒頭に「読む前の説明」を積まない。文字圧を下げ、すぐニュース本文と図解に入る。

- 冒頭に残す：**タイトル → 短いリード文（1〜2 文）→ サムネ or 最初の図解 1 枚 → そのまま本文の最初の H2**
- 冒頭に **連続して積まない**：`3行でわかるまとめ` / `この記事で整理すること` / `先に結論` / `今押さえるべき N 点` / `この記事の前提`
- 出典・但し書き（「この記事の前提」相当）は、冒頭ではなく **本文末「参考情報」付近に「この記事の注意点」として短く** 置く
- 本文中の **まとめ・比較表・チェックリスト・判断ガイドは残してよい**（冒頭への連打だけをやめる）
- 既存記事も同方針で順次見直す。生成ルール（`scripts/sumahon/generate-article-prompt.mjs`）も Zone 1 を軽い冒頭に更新済み。詳細: [`docs/article_structure_templates.md`](docs/article_structure_templates.md)

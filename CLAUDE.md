...
- **本投稿に記事リンクを入れない。** リンクは必ずリプライ側。本投稿は画像＋短文＋ハッシュタグ2〜4個（カテゴリ1＋`#すまラボ`＋X検索で生存確認済みの題材最大2）。題材なしの2個投稿を正常系とする。
...
**投稿文ルール：** 短め / 本投稿に URL は入れない（リンクはリプライ） / 題材タグ候補は投稿直前にXの「最新」で1回検索し、過去7日以内に3投稿以上・3アカウント以上を確認できた候補だけ最大2個採用 / 確認できなければカテゴリ＋`#すまラボ`の2個で投稿 / 空白・ハイフン除去等で複数語を連結した造語タグは禁止（モデル名は`#Claude`等の短い既存タグへ） / `#Google`・`#Apple`等の企業名単体・巨大一般タグは候補外 / 「普通の人」表現禁止 / 煽らない / 記事内容に沿う

...
- **ハッシュタグはブランド #すまラボ 常時1個＋流入タグ0〜2個（合計最大3、通常1〜2）**。カテゴリタグとコードによる候補生成は廃止。投稿直前のブラウザ経路が自然検索語でXを1回検索し、検索結果の投稿で実際に使われているタグを収集・集計して最多を採用する。本文1行目には同じ自然検索語を置き、--search-phrase と --discovered-traffic-tags で確定する。

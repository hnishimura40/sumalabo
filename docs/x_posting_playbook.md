...
- ハッシュタグは**ブランド #すまラボ 常時1個＋流入タグ0〜2個（合計最大3、通常1〜2）**。カテゴリタグは廃止。投稿直前に読者の自然検索語でXを1回検索し、結果内で実際に使われているタグを収集・集計して最多を採用する。見つからなければ流入タグなし。本文1行目には同じ自然検索語を含める（2026-07-28、第3次改修）。

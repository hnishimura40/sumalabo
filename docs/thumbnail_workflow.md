...
重要: 記事サムネイルは毎回ChatGPT側で作成します。Codexはサムネイルを自動生成しません。Codexは、作成済みサムネイルを指定パスに配置し、記事frontmatterに反映する役割を担当します。
...
- ChatGPT側で画像を作成する。
- Codexは画像生成を行わない。
- Codexは受け取った画像ファイルを整理し、必要に応じてWebP化・配置・frontmatter反映を行う。
- サムネイルは記事内容と一致させる。

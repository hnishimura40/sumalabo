...
新デプロイを検知して「新しいバージョンがあります」バナーを出す方式は未実装。
現状 `sw.js` はビルドごとに内容が変わらないため **SW の updatefound では検知できない**。
実装するなら「ビルドIDを持つ `/version.json` を出力し、`visibilitychange`/focus で
ポーリングして比較 → バナー → ワンタップで本機能の強制更新を呼ぶ」方式が最小コスト。

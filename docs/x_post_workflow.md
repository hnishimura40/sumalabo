...
検証スニペットは `scripts/sumahon/x-post-verify.mjs`（`postExistsJs(uniqueText)` / `REPLY_COUNT_JS` / `MEDIA_COUNT_JS` / `SEND_READY_JS`）。javascript_tool にそのまま貼って使う。
...
2. 返信 / `tweetButtonInline` を MCP click（座標クリックがスケールでずれる場合は要素 `.click()` でも可）。
3. **5 秒待機**。
4. `REPLY_COUNT_JS` を再取得。
   - **返信数 N→N+1 かつ `composerCleared:true`** → 確定。**この直後にリプライを台帳へ書く**。
...
- **本投稿が確定した直後**: `data/automation/ledger.json` に `status: x_posted` / `xPostUrl` を書く。加えて dedup 台帳 `node scripts/run/post-to-x.mjs --record --slug <slug> --postUrl <本投稿URL>`。
- **リプライが確定した直後**: `xReplyUrl` を追記（本投稿とリプライを**別々に**記録する。片方だけ確定した状態も台帳に正しく残す）。

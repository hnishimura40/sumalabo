...
- **本投稿**: 画像4枚＋短い本文＋ハッシュタグ。ブランドタグ #すまラボ は常時1個。流入タグは投稿直前のX話題検索結果で実際に最多の0〜2個だけ。カテゴリタグは廃止し、合計最大3個（通常1〜2個）。本文1行目には読者の自然検索語を含める。**本投稿に記事リンクを入れない。**
...
- **流入タグは話題検索結果から発見する** — 投稿直前に自然検索語でXを1回検索し、結果内の実在タグを収集・集計して最多を最大2個採用。0個を正常系とし、候補タグ生成・カテゴリタグは使わない。本文1行目には検索に使った自然語を入れる

...
export function validatePerformanceBlock(text = "") {
  if (!/^##\s+演出ブロック(?:\s|（|\(|$)/m.test(text)) return { ok: false, missing: ["演出ブロック"] };
  const required = ["衣装", "小道具", "手・小道具", "ポーズ", "背景", "表情", "演出根拠", "日付型判定", "主要モチーフ", "直近10記事類型チェック"];
  const missing = required.filter((label) => !text.includes(label));
  const outfitLine = text.match(/^\s*-\s*(?:\*\*)?衣装[^:：]*[:：](.*)$/m)?.[1]?.trim() || "";
  const standardOnly = /(?:正本準拠|標準)(?:の)?白衣|標準衣装/.test(outfitLine)
    && !/(?:テーマ|風|アクセ|バッジ|チャーム|ストラップ|ジャケット|ベスト|ヘッドセット|帽子|肩掛け|首かけ)/.test(outfitLine);
  if (standardOnly && !/中立解説系.*(?:理由|ため)|中立.*(?:理由|ため)/.test(outfitLine)) {
    missing.push("テーマ別衣装（標準衣装は中立解説系の理由明記時のみ）");
  }
  const handLine = text.match(/^\s*-\s*(?:\*\*)?手・小道具[^:：]*[:：](.*)$/m)?.[1]?.trim() || "";
  if (!/(?:自然な構図|壊れにくい手)/.test(handLine)) {
    missing.push("手・小道具の『壊れにくい手を自然に描く』既定");
  }
  if (!/(?:開いた手|軽く添える手)/.test(handLine)) {
    missing.push("手・小道具の低リスクな自然手指定");
  }
  const dateType = text.match(/^\s*-\s*(?:\*\*)?日付型判定[^:：]*[:：](.*)$/m)?.[1]?.trim() || "";
  const mainMotif = text.match(/^\s*-\s*(?:\*\*)?主要モチーフ[^:：]*[:：](.*)$/m)?.[1]?.trim() || "";
  const recentCheck = text.match(/^\s*-\s*(?:\*\*)?直近10記事類型チェック[^:：]*[:：](.*)$/m)?.[1]?.trim() || "";
  if (/該当/.test(dateType) && !/非該当/.test(dateType)) {
    if (!/^\s*-\s*(?:\*\*)?変化の中身[^:：]*[:：].+/m.test(text)) missing.push("日付型記事の『変化の中身』");
    if (!/^\s*-\s*(?:\*\*)?日付ラベル[^:：]*[:：].+/m.test(text)) missing.push("日付型記事の短い日付ラベル");
    if (/カレンダー|日めくり|スケジュール帳/.test(mainMotif)) {
      missing.push("日付型記事の主要モチーフはカレンダー類禁止");
    }
  }
  if (!/(?:類似なし|重複なし|異なる|変更)/.test(recentCheck)) {
    missing.push("直近10記事との類型比較結果と、類似時の構図変更");
  }
...
function assistedInstruction(state, step) {
  ...
  const map = {
    ...
    chatgpt_turn2_selection: `同チャットで Editorial Selection（採用/限定採用/不採用）→ ${dir}/editorial_selection.md に保存。カテゴリ候補は「${state.category?.name || "ニュースをかみくだく"}」。当サイトの実体験・検証が記事の核なら hands-on を選ぶ。**docs/general_reader_explainer_policy.md の『生成前の読者翻訳設計』を適用し、必ず \`## 読者翻訳設計\` を追加する。主題の一言説明、関係がある人、あまり関係がない人、全専門語の \`専門語 → 普通の日本語による短い説明\`、事実から言えること/言えないことを記録する。これは内部成果物であり公開本文へコピーしない。**`,
    chatgpt_turn3_draft: `同チャットで本文ドラフト → ${dir}/draft_article.md に保存。**docs/general_reader_explainer_policy.md を正本として、専門ニュースの要約ではなく、その言葉を初めて聞く人への説明にする。順序は『これは何か→自分に関係するか→何が変わったか→何が便利か→注意点→詳しい情報』。30秒欄は『これは何？／何があった？／誰に関係ある？／結局どうなの？』の4項目。専門語は初出で普通の日本語に直し、読者翻訳設計の全用語を回収する。**`,
    chatgpt_turn4_review: `同チャットでセルフレビュー → ${dir}/review_report.md に保存。日付数値整合 / 煽り断定 / 禁則語 / 線引き / 旧情報残存に加え、**(1)主題の説明が冒頭にある、(2)30秒欄4項目だけで理解できる、(3)関係がある人/薄い人が分かる、(4)読者翻訳設計の専門語が初出で説明される、(5)キャラが読者の疑問を代弁する、(6)噛み砕きが長文化していない、(7)スカウト元・コピー指示・自動公開・veto・内部用・生成prompt等が公開文に無い**ことを項目別に合否判定する。`,
    chatgpt_turn5_final: `レビュー反映の確定稿 → ${dir}/final_article.md に保存。レビューの一般読者向け7項目に1つでも不合格が残る場合は保存せず修正する。事実→一般向け説明の順序を守り、不明点は断定しない。`,
    chatgpt_turn6_slideplan: `slide_plan（本文スライド8枚 4:5 1280×1600 + サムネ16:9 体験図方針）→ ${dir}/slide_plan.md に保存。サムネは assets/characters/character-sheet.md の「体験図」3型から選ぶこと。**先頭付近に必ず \`## 演出ブロック\` を設け、\`衣装\`・\`小道具\`・\`ポーズ・動き\`・\`背景・状況\`・\`演出根拠\` の5欄を記事テーマから自動生成する。さらに \`スライド演出方針\` として、本文8枚で使う小道具・動き・背景（必要なら一貫したテーマ衣装）を記す。** サムネイル節には \`感情\`・\`行動\`・\`場面\`・\`手の動作\` を独立した必須欄として書く。感情は記事主題に対する驚き/困惑/発見/感心/呆れ/迷い/喜び等を具体化し、行動はスマホ/PCを使う、選択肢を見比べる、覗き込む、計算・メモをする等から記事固有のものを最低1つ指定する。背景・小物・キャラクターは同じ状況の中に置き、立つだけ・微笑むだけ・テキストパネル横の紹介役は禁止する。スマホ保持・単純な指さしは許容し、手の接写・細かな操作・複雑な指形は避ける。文字は顔・胴体・動作と干渉しない余白へ一枚絵として生成する。 同一性は「顔・体型・髪型・耳ビレ・首輪・アンテナ・ハート」の正本アンカーに限定して固定し、衣装・小道具・ポーズ・背景は演出ブロックに従い自由に変える。**衣装は記事テーマから必ず具体化し、標準衣装を許すのは中立解説系で標準にする理由を衣装欄へ明記した場合だけ。** サムネは標準衣装＋棒立ちを禁止し、記事テーマから連想される衣装・小道具・シチュエーションを最低1つ入れる。スライドも置物化を避け、比較なら2つを見比べる、検証なら道具で調べる等の動きを付ける。スライドで衣装を変える場合は8枚で一貫させる（露出過多・無関係なコスプレNG）。**日付・期日・変更日が主題に含まれる記事は、カレンダー・日めくり・スケジュール帳を主要モチーフにしない。日付ではなく「何がどう変わるか」を人物の行動と場面で演じ、日付は「8/26から」のような短い文字ラベルだけにする。消える・有料化・増量・発表待ち・条件変更等の例は発想の補助であり、同じ棚・箱・分岐路を使い回して新しい固定型にしてはならない。** slide_plan作成前に公開日順の直近10記事のthumbnailAltと利用可能なslide_planを確認し、同じ類型なら主要モチーフ・場所・人物の動きを変更する。演出ブロックへ \`日付型判定\`・\`変化の中身\`・\`主要モチーフ\`・\`日付ラベル\`・\`直近10記事類型チェック\` を記録する。例:
\`## 演出ブロック\`
\`- 衣装: 検証用の作業ベスト\`
\`- 小道具: 虫眼鏡、工具、対象機器\`
...
**漢字化け対策（docs/kanji_pitfalls.md）: スライドの帯・見出し・吹き出しの短い文言は、化けやすい漢字（目/未/末/微 等）を避け、ひらがな・言い換えを優先する（例「目的別」→「使い方で」）。特に1〜4文字のラベルは優先的にやさしい和語にする。** **X直接投稿を見据え、各スライドは「文字量を絞り・数字は正確に・単体で意味が通る」ことを意識（文字密度の高いスライドは X 選抜から外れ記事内専用になる）。**`,
  ...
  **本文は v3 コンポーネントで組む（docs/article_components_v3.md 準拠・2026-07-08 有効化。お手本: content/articles/202607-claude-fable-5-free-extension-july13.mdx）:**
  ...
  - 冒頭に <Summary30>（1記事1回）。4項目をこの順で必須: **これは何？ / 何があった？ / 誰に関係ある？ / 結局どうなの？**
  ...
  - **図解スライド8枚（article-slide-section + slide-reading-note + ライトボックス）と「## 参考情報」（URL2件以上）は従来どおり併用**（v3 はこれらを置き換えない）。slide-reading-noteに『図のポイント（コピー可能）』等の制作ラベルは付けず、読者向け補足だけを書く。`,
  };
  map.chatgpt_turn6_slideplan += `\n**演出ブロックの追加必須欄**: \`手・小道具\` は「壊れにくい手を自然に描く」を既定にする。自然な構図を優先し、手を隠すことを既定にしない。開いた手・軽く添える手を優先し、手は小さめ・遠め、動作する手は片手だけ、もう片方は体側で自然に休ませる。物を強く握る、両手で別々の動作をする、指を複雑に組む・数える、手のクローズアップは避ける。小道具が不可欠な場合だけ片手で軽く持つ。手を消すためだけの後ろ手・机の陰・フレームアウトは使わない。テーマ衣装・小道具・姿勢・視線・背景を削らず、標準衣装・棒立ちへ退避しない。\`表情\` も記事の感情トーンから自動導出する。身体構造は、ひまり=腕/手/脚各2、らぼまる=腕・手・脚・足が左右各1つ（追加突起はアンテナ1本と左右の耳ビレのみ、腕は短く・太く・丸い）を前提にする。`;

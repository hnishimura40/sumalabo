...
export function validatePerformanceBlock(text = "") {
  ...
  if (!/手(?:を|は)原則描かない/.test(handLine)) {
    missing.push("手・小道具の『手を原則描かない』既定");
  }
  if (!/(?:卓上スタンド|机上配置|ポケット|長い袖|後ろ手|前景オブジェクト|フレームアウト)/.test(handLine)) {
    missing.push("手・小道具の具体的な退避方法");
  }
...
function assistedInstruction(state, step) {
  ...
  const map = {
    ...
    chatgpt_turn6_slideplan: `slide_plan（本文スライド8枚 4:5 1280×1600 + サムネ16:9 体験図方針）→ ${dir}/slide_plan.md に保存。サムネは assets/characters/character-sheet.md の「体験図」3型から選ぶこと。**先頭付近に必ず \`## 演出ブロック\` を設け、\`衣装\`・\`小道具\`・\`ポーズ・動き\`・\`背景・状況\`・\`演出根拠\` の5欄を記事テーマから自動生成する。さらに \`スライド演出方針\` として、本文8枚で使う小道具・動き・背景（必要なら一貫したテーマ衣装）を記す。** 同一性は「顔・体型・髪型・耳ビレ・首輪・アンテナ・ハート」の正本アンカーに限定して固定し、衣装・小道具・ポーズ・背景は演出ブロックに従い自由に変える。**衣装は記事テーマから必ず具体化し、標準衣装を許すのは中立解説系で標準にする理由を衣装欄へ明記した場合だけ。** サムネは標準衣装＋棒立ちを禁止し、記事テーマから連想される衣装・小道具・シチュエーションを最低1つ入れる。スライドも置物化を避け、比較なら2つを見比べる、検証なら道具で調べる等の動きを付ける。スライドで衣装を変える場合は8枚で一貫させる（露出過多・無関係なコスプレNG）。例:
\`## 演出ブロック\`
\`- 衣装: 検証用の作業ベスト\`
\`- 小道具: 虫眼鏡、工具、対象機器\`
...
**漢字化け対策（docs/kanji_pitfalls.md）: スライドの帯・見出し・吹き出しの短い文言は、化けやすい漢字（目/未/末/微 等）を避け、ひらがな・言い換えを優先する（例「目的別」→「使い方で」）。特に1〜4文字のラベルは優先的にやさしい和語にする。** **X直接投稿を見据え、各スライドは「文字量を絞り・数字は正確に・単体で意味が通る」ことを意識（文字密度の高いスライドは X 選抜から外れ記事内専用になる）。**`,
  ...
  **本文は v3 コンポーネントで組む（docs/article_components_v3.md 準拠・2026-07-08 有効化。お手本: content/articles/202607-claude-fable-5-free-extension-july13.mdx）:**
  ...
  map.chatgpt_turn6_slideplan += `\n**演出ブロックの追加必須欄**: \`手・小道具\` は「手を原則描かない」を既定にする。卓上スタンド・机上配置・ポケット・長い袖・後ろ手・前景オブジェクト・フレームアウトから具体的な退避方法を各画像に指定し、手首から先を隠す。情報上どうしても手が必要な場合だけ例外として片手/両手のどちらで持つかと空いている手の位置を明記し、手は小さく、クローズアップしない。同じ人物へ「片手で持つ＋反対の手で指す」など両手の同時別動作を指定せず、小道具へ腕を伸長させない。ただし、この安全指定を理由に衣装・小道具・ポーズ・背景を削らない。手を使わない演出として、(1)テーマ別衣装、(2)傍ら・机上・卓上スタンドの小道具、(3)首かけ・肩掛け・バッジ・ヘッドセット、(4)ポケット・長い袖・後ろ手・机・操作卓・前景オブジェクト・フレームアウト、(5)覗き込む・見上げる・振り返る等の姿勢と視線、(6)テーマ固有の背景の作り込み、から最低2種類を選ぶ。標準衣装・棒立ちへ退避しない。\`表情\` も記事の感情トーンから自動導出する。身体構造は、ひまり=腕/手/脚各2、らぼまる=腕・手・脚・足が左右各1つ（追加突起はアンテナ1本と左右の耳ビレのみ、腕は短く・太く・丸い）を前提にする。`;

...
- 手・小道具: 自然な構図を優先し、開いた手・軽く添える手を小さめ・遠めに描く。動作する手は片手だけ、もう片方は体側で自然に休ませる。強い把持、両手の別動作、指を組む・数えるポーズ、クローズアップを避ける
- ポーズ・動き: 覗き込む・見上げる・振り返る・身を乗り出す・画面外へ視線を送る等、手を主役にしない姿勢と視線で主題を表す
- 背景・状況: テーマ固有の現場を、奥行き・照明・机上機器・環境小物まで作り込む
- 演出根拠: 対象セクションの中心名詞から導出する
- 日付型判定: 非該当
- 変化の中身: 非該当
- 主要モチーフ: 対象セクション固有の作業・対象物
- 日付ラベル: なし
- 直近10記事類型チェック: 汎用補完のため類似なし`;
...
【身体構造（公開ブロック・最重要）】
- 生成の既定は「壊れにくい手を自然に描く」。手を隠すことを既定にせず、開いた手・軽く添える手を優先する。手は小さめ・遠め、動作する手は片手だけにし、もう片方は体側で自然に休ませる。
...
- 小道具が必要な場合は既存の片手で軽く添えるか軽く持ち、空いている手は体側で自然に休ませる。両手持ちは避ける。小道具や操作対象は自然に手が届く距離へ置き、構図の都合で腕を伸長させない。
- 物を強く握る、両手で別々の動作をする、指を複雑に組む・数えるポーズ、手のクローズアップを避ける。小道具が不可欠な場合だけ片手で軽く持つ。
...
- 手を隠しても、テーマ衣装、小道具の配置、姿勢と視線、テーマ固有の背景のうち最低2種類を必ず強く残す。「安全のため演出を減らす」は不可。
...
- サムネは、記事主題に対する具体的な感情、題材へ働きかける行動、背景・小物・人物が同居する場面として描く。立つだけ、微笑むだけ、テキストパネル横の紹介役は禁止する。
- 日付・期日・変更日を含むサムネでは、カレンダー・日めくり・スケジュール帳を主要モチーフにしない。日付ではなく演出ブロックの「変化の中身」を人物の行動・対象物・場所で描き、日付は短い文字ラベルだけにする。カレンダー類は片隅の小道具に限る。
- 消える・有料化・増量・発表待ち・条件変更の例示構図をそのまま反復しない。演出ブロックの「直近10記事類型チェック」に従い、類似時は主要モチーフ・場所・人物の動きを変える。
- スマホ/PCを実際に使う、選択肢を見比べる、覗き込む、計算・メモをする等、対象セクションで指定された記事固有の動作を必ず描く。スマホ保持・単純な指さしは許容し、手の接写・細かな操作・複雑な指形は避ける。
- 文字を含む場合は文字込み一枚絵として生成し、顔・胴体・動作を覆わない余白を場面内へ確保する。文字なしベース画像への後乗せを前提にしない。
...
演出ブロックと身体構造ルールが競合して見える場合は、身体構造を守りつつ、開いた手・軽く添える手・片手だけの単純動作へ置き換えて演出密度を維持する。手を隠す構図や演出項目の削除へ退避してはならない。

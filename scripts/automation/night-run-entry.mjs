...
async function main() {
  ...
  try { JSON.parse(readFileSync(ENVIRONMENT_FILE, "utf8").replace(/^\uFEFF/u, "")); }
  catch (error) { fail(runId, startedAt, "runner_environment_definition_invalid", error.message); }
  if (process.exitCode !== 30) {
    try {
      const hygiene = inspectRunnerHygiene({ root: ROOT, environmentFile: ENVIRONMENT_FILE, cleanup: true });
      if (!hygiene.ok) fail(runId, startedAt, hygiene.reason, String(hygiene.gitError || hygiene.dangerous.join("\n")).slice(0, 2000));
    } catch (error) {
      fail(runId, startedAt, "runner_cleanup_allowlist_failed", error.message);
    }
  }
  if (process.exitCode !== 30) {

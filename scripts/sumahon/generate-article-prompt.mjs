...
**[Zone 1: 入口ゾーン] — 冒頭は軽く。説明ブロックを重ねない**
1. **短いリード文 1〜2 文**（タイトルの再掲ではなく、何のニュースかを一言で）
2. **最初の図解スライド or サムネ 1 枚**（\`<section class="article-slide-section">\` など）を入口に置く
3. その直後に **本文の最初の H2 へ入る**

> 冒頭に「3行でわかるまとめ」「この記事で整理すること」「先に結論」「今押さえるべき N 点」「この記事の前提」などの説明ボックスを **連続して積まない**。読む前の説明を減らし、すぐニュース本文と図解に入る。要約・比較表・チェックリストは本文中で使ってよい（冒頭に連打しないだけ）。出典・但し書きは記事末尾「参考情報」付近に短く置く。
...
具体的な書き方:
1. 本文冒頭は **短いリード文 1〜2 文**（タイトル再掲は不可）から始める。説明ボックスを冒頭に連打しない
2. その直後に **最初の図解スライド or サムネ 1 枚** を入口として置く
3. そのまま **本文の最初の H2** に入る（「3行でわかるまとめ」「この記事で整理すること」「先に結論」「今押さえるべき N 点」「この記事の前提」を冒頭に積まない。要約・比較は本文中で使い、出典・但し書きは末尾「参考情報」付近へ短く）
4. 本文中盤で「判断軸 (${understanding.buyWaitOrWatch || "買う / 待つ / 様子見"})」を比較表または判断カードで表現する（ただし記事の主軸は「ニュースをやさしく噛み砕く」。判断は補助）

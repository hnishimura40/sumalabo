...
export function derivePerformanceBlock({ theme = "本記事", variant = "news" } = {}) {
  ...
  const recipe = PERFORMANCE_RECIPES
    ...
    thumbnail: {
      ...
      handUse: "自然な構図を優先し、手を隠すことを既定にしない。開いた手・軽く添える手を優先し、手は小さめ・遠めに描く。動作する手は片手だけにし、もう片方は体側で自然に休ませる。物を強く握る、両手で別々の動作をする、指を複雑に組む・数える、手のクローズアップは避ける。小道具が不可欠な場合だけ片手で軽く持つ。手を消すためだけの後ろ手・机の陰・フレームアウトは使わない。テーマ衣装・小道具・姿勢・視線・背景を削らず、標準衣装・棒立ちへ退避しない",
  ...
  return {
    ...
    thumbnail: {
      ...
      handUse: `自然な構図を優先し、手を隠すことを既定にしない。開いた手・軽く添える手を優先し、手は小さめ・遠めに描く。動作する手は片手だけにし、もう片方は体側で自然に休ませる。物を強く握る、両手で別々の動作をする、指を複雑に組む・数える、手のクローズアップは避ける。小道具が不可欠な場合だけ片手で軽く持つ。手を消すためだけの後ろ手・机の陰・フレームアウトは使わない。テーマ衣装・小道具・姿勢・視線・背景を削らず、標準衣装・棒立ちへ退避しない。${recipe.thumbnail.handUse || "小道具が必要なら既存の片手を軽く添え、空いている手は体側で自然に休ませる"}`,
      pose: `${recipe.thumbnail.pose}。このテーマ固有の動作と視線を維持し、片手だけを開くか小道具へ軽く添え、もう片方は体側で自然に休ませる`,
      handsFreeAlternatives: [
        "テーマ別衣装",
        "傍ら・机上の小道具",
        "開いた手を体の近くで自然に見せる",
        "片手を小道具へ軽く添え、もう片方は体側で休ませる",
        "首かけ・肩掛け・バッジ・ヘッドセット",
        "覗き込む・見上げる・振り返る姿勢と視線",
    ...
    slides: {
      ...
      handUse: "自然な構図を優先し、手を隠すことを既定にしない。開いた手・軽く添える手を優先し、手は小さめ・遠めに描く。動作する手は片手だけにし、もう片方は体側で自然に休ませる。物を強く握る、両手で別々の動作をする、指を複雑に組む・数える、手のクローズアップは避ける。小道具が不可欠な場合だけ片手で軽く持つ。手を消すためだけの後ろ手・机の陰・フレームアウトは使わない。テーマ衣装・小道具・姿勢・視線・背景を削らず、標準衣装・棒立ちへ退避しない",
      handsFreeAlternatives: ["開いた手", "軽く添える手", "片手だけを動かす", "小さめ・遠め", "姿勢と視線", "背景の作り込み"],
      expression: expression.direction,
      pose: "比較・確認・操作などの役割は、覗き込む・見上げる・振り返る・身を乗り出す姿勢と視線で表す。手は自然に見せ、片手だけを開くか小道具へ軽く添え、もう片方は体側で休ませる",

...
export function derivePerformanceBlock({ theme = "本記事", variant = "news" } = {}) {
  ...
  const recipe = PERFORMANCE_RECIPES
    ...
    thumbnail: {
      ...
      handUse: "手は原則描かない。卓上スタンド、ポケット、長い袖、後ろ手、前景オブジェクト、フレームアウトを優先し、手首から先を見せない。小道具は机上・傍ら・スタンドへ配置する。情報上どうしても手が必要な場合だけ例外として片手持ち／両手持ちを固定し、手は小さくする。同じ人物の両手に別々の動作を同時指定せず、腕を伸長させない",
  ...
  return {
    ...
    thumbnail: {
      ...
      handUse: `手は原則描かない。卓上スタンド・ポケット・長い袖・後ろ手・前景オブジェクト・フレームアウトで手首から先を隠す。小道具は机上・傍ら・独立スタンドへ置く。${recipe.thumbnail.handUse || "情報上どうしても手が必要な場合だけ、例外として片手持ち／両手持ちを固定し、空いている手は体側に置く"}。手のクローズアップを避け、見せる場合も小さくして画面の主役にしない。同じ人物の両手に別々の動作を同時指定しない。例外の小道具は自然に届く距離へ置き、腕を伸長させない。安全指定を理由にテーマ衣装・小道具・姿勢・視線・背景を削らず、標準衣装・棒立ちへ退避しない`,
      pose: `${recipe.thumbnail.pose}。このテーマ固有の動作と視線を維持したまま、手はポケット・長い袖・後ろ手・前景・フレーム外へ逃がす`,
      handsFreeAlternatives: [
        "テーマ別衣装",
        "傍ら・机上の小道具",
        "卓上スタンド・前景オブジェクト・フレームアウト",
        "ポケット・長い袖・後ろ手で手首から先を隠す",
        "首かけ・肩掛け・バッジ・ヘッドセット",
        "覗き込む・見上げる・振り返る姿勢と視線",
    ...
    slides: {
      ...
      handUse: "手は原則描かない。卓上スタンド・机上配置・ポケット・長い袖・後ろ手・前景オブジェクト・フレームアウトで手首から先を隠す。情報上どうしても手が必要な小道具だけ例外として片手持ち／両手持ちを明記し、手は小さく、画面の主役にしない。同じ人物の両手に別々の動作を同時指定しない。小道具は自然に届く距離へ置き、腕を伸長させない",
      handsFreeAlternatives: ["身につける小道具", "卓上スタンド・机上配置", "ポケット・長い袖・後ろ手", "前景オブジェクト・フレームアウト", "姿勢と視線", "背景の作り込み"],
      expression: expression.direction,
      pose: "比較・確認・操作などの役割は、覗き込む・見上げる・振り返る・身を乗り出す姿勢と視線で表す。手は原則隠し、必要な場合だけ例外として小さく見せる",

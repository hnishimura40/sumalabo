...
12. **表情・トーンが記事の性質と矛盾していないか（サムネ・惜別記事で特に重要）** — 記事テーマが **終了・サービス終了・障害・不具合・値上げ・リコール・脆弱性** など「注意・惜別」系なのに、キャラが**満面の笑み・はしゃいでいる・ワクワク顔**になっている場合は needs_revision（blocking）。注意・惜別系は「真剣／少し困り顔／落ち着いて対処している顔」＋「確認する・メモを取る・カレンダーを指す・退避作業をする」等の対処行動がふさわしい（炎・涙・パニックの煽りも不可）。逆にポジティブ系（新機能・復活・延長・無料化）で無表情・暗すぎるのも軽微な不整合として指摘してよい（warning）。判定基準は `assets/characters/character-sheet.md` の「記事の感情トーンを表情・仕草に反映する」。

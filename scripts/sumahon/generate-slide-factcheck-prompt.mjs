...
10. **キャラの視覚的破綻がないか（認識アンカー一致・最重要）** — 判定は「**認識アンカー**」で行う。ひまりのアンカー = **金髪サイドテール + 水色/ティールのリボン、青い瞳、正本の顔立ち・頭身**。らぼまるのアンカー = **白い卵型ボディ + 頭のアンテナ（黄緑の玉）+ 黒い丸い目 + 胸のオレンジのハートボタン + 正本の体型**。**このアンカーが逸脱している場合（髪色・髪型・顔立ち・頭身が違う／人型メカロボット化／らぼまるのボディ形状・アンテナ・ハートバッジが違う 等）は needs_revision（blocking）**。同一記事の別スライドでアンカーが変わる（途中からキャラが変わる）のも blocking。工房チャットの参照劣化で後半スライドから崩れる典型パターンを必ず疑う。
    - ★ **服・上着・小道具の違いそのものは「破綻」ではない**（衣装はテーマで変わりうる）。アンカーが保たれていれば別人化とはみなさない。ただし**スライド8枚は標準衣装（白ワンピ+白パーカー/ティール差し色）で一貫**させるのが原則なので、スライド間で衣装がバラつく場合は warning（"ok_with_warning"）とする（衣装バリエーションはサムネ専用）。
